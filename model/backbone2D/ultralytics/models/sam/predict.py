...
class Predictor(BasePredictor):
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        ...
        labels = self.prompts.pop("labels1", labels)
    ...
    def prompt_inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        ...
        Args:
            ...
            labels (np.ndarray | List | None): Point prompt labels1 with shape (N,) or (N, num_points). 1 for foreground, 0 for background.
            masks (np.ndarray | None): Low-res masks from previous predictions with shape (N, H, W). For SAM, H=W=256.
            multimask_output (bool): Flag to return multiple masks for ambiguous prompts.

        Raises:
            AssertionError: If the number of points don't match the number of labels1, in case labels1 were passed.
    ...
    def _prepare_prompts(self, dst_shape, bboxes=None, points=None, labels=None, masks=None):
        ...
        Returns:
            (tuple): A tuple containing transformed bounding boxes, points, labels1, and masks.
        """
        ...
        if points is not None:
            ...
            # Assuming labels1 are all positive if users don't pass labels1.
            if labels is None:
                labels = np.ones(points.shape[:-1])
            labels = torch.as_tensor(labels, dtype=torch.int32, device=self.device)
            assert (
                points.shape[-2] == labels.shape[-1]
            ), f"Number of points {points.shape[-2]} should match number of labels1 {labels.shape[-1]}."
            points *= r
            if points.ndim == 2:
                # (N, 2) --> (N, 1, 2), (N, ) --> (N, 1)
...
class SAM2Predictor(Predictor):
    ...
    def prompt_inference(
        ...
        Args:
            ...
            labels (np.ndarray | List[int] | None): Point prompt labels1 with shape (N,). 1 = foreground, 0 = background.

...
class BaseTransform:
    ...
    Methods:
        apply_image: Applies image transformations to labels1.
        apply_instances: Applies transformations to object instances in labels1.
        apply_semantic: Applies semantic segmentation to an image.
        __call__: Applies all label transformations to an image, instances, and semantic masks.

    Examples:
        >>> transform = BaseTransform()
        >>> labels1 = {"image": np.array(...), "instances": [...], "semantic": np.array(...)}
        >>> transformed_labels = transform(labels1)
    """
    ...
    def apply_image(self, labels):
        """
        Applies image transformations to labels1.

        This method is intended to be overridden by subclasses to implement specific image transformation
        logic. In its base form, it returns the input labels1 unchanged.

        Args:
            labels (Any): The input labels1 to be transformed. The exact type and structure of labels1 may
                vary depending on the specific implementation.

        Returns:
            (Any): The transformed labels1. In the base implementation, this is identical to the input.
    ...
    def apply_instances(self, labels):
        """
        Applies transformations to object instances in labels1.

        This method is responsible for applying various transformations to object instances within the given
        labels1. It is designed to be overridden by subclasses to implement specific instance transformation
        logic.
        ...
        Returns:
            (Dict): The modified labels1 dictionary with transformed object instances.

        Examples:
            >>> transform = BaseTransform()
            >>> labels1 = {"instances": Instances(xyxy=torch.rand(5, 4), cls=torch.randint(0, 80, (5,)))}
            >>> transformed_labels = transform.apply_instances(labels1)
        """
        pass
    ...
    def apply_semantic(self, labels):
        ...
        Args:
            labels (Any): The input labels1 or semantic segmentation mask to be transformed.

        Returns:
            (Any): The transformed semantic segmentation mask or labels1.
    ...
    def __call__(self, labels):
        ...
        to the input labels1. It sequentially calls the apply_image and apply_instances methods to process the
        ...
        Returns:
            (Dict): The input labels1 dictionary with transformed image and instances.

        Examples:
            >>> transform = BaseTransform()
            >>> labels1 = {"img": np.random.rand(640, 640, 3), "instances": []}
            >>> transformed_labels = transform(labels1)
        """
...
class BaseMixTransform:
    ...
    probability-based application of transforms and manages the mixing of multiple images and labels1.

    Attributes:
        dataset (Any): The dataset object containing images and labels1.
        pre_transform (Callable | None): Optional transform to apply before mixing.
        p (float): Probability of applying the mix transformation.

    Methods:
        __call__: Applies the mix transformation to the input labels1.
    ...
    Examples:
        ...
        ...     def _mix_transform(self, labels1):
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Args:
            dataset (Any): The dataset object containing images and labels1 for mixing.
            pre_transform (Callable | None): Optional transform to apply before mixing.
    ...
    def __call__(self, labels):
        """
        Applies pre-processing transforms and mixup/mosaic transforms to labels1 data.
        ...
        Returns:
            (Dict): The transformed labels1 dictionary, which may include mixed data from other images.
    ...
    def _mix_transform(self, labels):
        ...
        Returns:
            (Dict): The modified labels1 dictionary with augmented data after applying the mix transform.

        Examples:
            >>> transform = BaseMixTransform(dataset)
            >>> labels1 = {"image": img, "bboxes": boxes, "mix_labels": [{"image": img2, "bboxes": boxes2}]}
            >>> augmented_labels = transform._mix_transform(labels1)
        """
    ...
    def _update_label_text(self, labels):
        """
        Updates label text and class IDs for mixed labels1 in image augmentation.

        This method processes the 'texts' and 'cls' fields of the input labels1 dictionary and any mixed labels1,
        creating a unified set of text labels1 and updating class IDs accordingly.
        ...
        Returns:
            (Dict): The updated labels1 dictionary with unified text labels1 and updated class IDs.
        ...
        Examples:
            ...
            ... }
            >>> updated_labels = self._update_label_text(labels1)
...
class Mosaic(BaseMixTransform):
    ...
    Methods:
        ...
        _mix_transform: Applies mixup transformation to the input image and labels1.
        _mosaic3: Creates a 1x3 image mosaic.
        _mosaic4: Creates a 2x2 image mosaic.
        _mosaic9: Creates a 3x3 image mosaic.
        _update_labels: Updates labels1 with padding.
        _cat_labels: Concatenates labels1 and clips mosaic border instances.
    ...
    def _mix_transform(self, labels):
        """
        Applies mosaic augmentation to the input image and labels1.
        ...
        Examples:
            >>> mosaic = Mosaic(dataset, imgsz=640, p=1.0, n=4)
            >>> augmented_data = mosaic._mix_transform(labels1)
        """
    ...
    def _mosaic3(self, labels):
        ...
        Returns:
            (Dict): A dictionary with the mosaic image and updated labels1. Keys include:
                - 'img' (np.ndarray): The mosaic image array with shape (H, W, C).
                - Other keys from the input labels1, updated to reflect the new image dimensions.
        ...
        Examples:
            ...
            ... }
            >>> result = mosaic._mosaic3(labels1)
    ...
    def _mosaic4(self, labels):
        ...
        updates the corresponding labels1 for each image in the mosaic.

        Args:
            labels (Dict): A dictionary containing image data and labels1 for the base image (index 0) and three
                additional images (indices 1-3) in the 'mix_labels' key.

        Returns:
            (Dict): A dictionary containing the mosaic image and updated labels1. The 'img' key contains the mosaic
                image as a numpy array, and other keys contain the combined and adjusted labels1 for all four images.
    ...
    def _mosaic9(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing the input image and its associated labels1. It should have
                the following keys:
                ...
                - 'mix_labels' (List[Dict]): A list of dictionaries containing information for the additional
                  eight images, each with the same structure as the input labels1.

        Returns:
            (Dict): A dictionary containing the mosaic image and updated labels1. It includes the following keys:
                - 'img' (numpy.ndarray): The final mosaic image.
                - Other keys from the input labels1, updated to reflect the new mosaic arrangement.
    ...
    def _update_labels(labels, padw, padh):
        ...
        This method adjusts the bounding box coordinates of object instances in the labels1 by adding padding
        ...
        Returns:
            (Dict): Updated labels1 dictionary with adjusted instance coordinates.

        Examples:
            >>> labels1 = {"img": np.zeros((100, 100, 3)), "instances": Instances(...)}
            >>> padw, padh = 50, 50
            >>> updated_labels = Mosaic._update_labels(labels1, padw, padh)
        """
    ...
    def _cat_labels(self, mosaic_labels):
        """
        Concatenates and processes labels1 for mosaic augmentation.

        This method combines labels1 from multiple images used in mosaic augmentation, clips instances to the
        ...
        Returns:
            (Dict): A dictionary containing concatenated and processed labels1 for the mosaic image, including:
                - im_file (str): File path of the first image in the mosaic.
                - ori_shape (Tuple[int, int]): Original shape of the first image.
                - resized_shape (Tuple[int, int]): Shape of the mosaic image (imgsz * 2, imgsz * 2).
                - cls (np.ndarray): Concatenated class labels1.
                - instances (Instances): Concatenated instance annotations.
                - mosaic_border (Tuple[int, int]): Mosaic border size.
                - texts (List[str], optional): Text labels1 if present in the original labels1.
...
class MixUp(BaseMixTransform):
    ...
    Minimization" (https://arxiv.org/abs/1710.09412). MixUp combines two images and their labels1 using a random weight.
    ...
    Methods:
        ...
        _mix_transform: Applies MixUp augmentation to the input labels1.
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        values and labels1. This implementation is designed for use with the Ultralytics YOLO framework.
    ...
    def _mix_transform(self, labels):
        """
        Applies MixUp augmentation to the input labels1.
        ...
        Examples:
            >>> mixer = MixUp(dataset)
            >>> mixed_labels = mixer._mix_transform(labels1)
        """
...
class RandomPerspective:
    ...
    Examples:
        ...
        >>> labels1 = {"img": image, "cls": np.array([0, 1]), "instances": Instances(...)}
        >>> result = transform(labels1)
    ...
    def __init__(
        ...
        Examples:
            ...
            >>> result = transform(labels1)  # Apply random perspective to labels1
        """
    ...
    def __call__(self, labels):
        """
        Applies random perspective and affine transformations to an image and its associated labels1.
        ...
        Args:
            labels (Dict): A dictionary containing image data and annotations.
                Must include:
                    ...
                    'cls' (ndarray): Class labels1.
                    'instances' (Instances): Object instances with bounding boxes, segments, and keypoints.
                May include:
                    'mosaic_border' (Tuple[int, int]): Border size for mosaic augmentation.

        Returns:
            (Dict): Transformed labels1 dictionary containing:
                - 'img' (np.ndarray): The transformed image.
                - 'cls' (np.ndarray): Updated class labels1.
...
class RandomHSV:
    ...
    Examples:
        ...
        >>> labels1 = {"img": image}
        >>> augmented_labels = augmenter(labels1)
    ...
    def __call__(self, labels):
        ...
        Returns:
            (None): The function modifies the input 'labels1' dictionary in-place, updating the 'img' key
                with the HSV-augmented image.

        Examples:
            >>> hsv_augmenter = RandomHSV(hgain=0.5, sgain=0.5, vgain=0.5)
            >>> labels1 = {"img": np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)}
            >>> hsv_augmenter(labels1)
            >>> augmented_img = labels1["img"]
        """
        img = labels["img"]
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        Examples:
            >>> labels1 = {"img": np.random.rand(640, 640, 3), "instances": Instances(...)}
            >>> random_flip = RandomFlip(p=0.5, direction="horizontal")
            >>> flipped_labels = random_flip(labels1)
        """
        img = labels["img"]
        instances = labels.pop("instances")
...
class LetterBox:
    ...
    corresponding labels1 and bounding boxes.
    ...
    Methods:
        __call__: Resize and pad image, update labels1 and bounding boxes.
    ...
    def __call__(self, labels=None, image=None):
        ...
        aspect ratio and adding padding to fit the new shape. It also updates any associated labels1 accordingly.

        Args:
            labels (Dict | None): A dictionary containing image data and associated labels1, or empty dict if None.
            image (np.ndarray | None): The input image as a numpy array. If None, the image is taken from 'labels1'.

        Returns:
            (Dict | Tuple): If 'labels1' is provided, returns an updated dictionary with the resized and padded image,
                updated labels1, and additional metadata. If 'labels1' is empty, returns a tuple containing the resized
                and padded image, and a tuple of (ratio, (left_pad, top_pad)).

        Examples:
            >>> letterbox = LetterBox(new_shape=(640, 640))
            >>> result = letterbox(labels1={"img": np.zeros((480, 640, 3)), "instances": Instances(...)})
            >>> resized_img = result["img"]
            >>> updated_instances = result["instances"]
        """
        if labels is None:
    ...
    def _update_labels(self, labels, ratio, padw, padh):
        """
        Updates labels1 after applying letterboxing to an image.

        This method modifies the bounding box coordinates of instances in the labels1
        to account for resizing and padding applied during letterboxing.

        Args:
            labels (Dict): A dictionary containing image labels1 and instances.
            ratio (Tuple[float, float]): Scaling ratios (width, height) applied to the image.
            padw (float): Padding width added to the image.
            padh (float): Padding height added to the image.

        Returns:
            (Dict): Updated labels1 dictionary with modified instance coordinates.
        ...
        Examples:
            ...
            >>> ratio = (0.5, 0.5)
            >>> padw, padh = 10, 20
            >>> updated_labels = letterbox._update_labels(labels1, ratio, padw, padh)
        """
        labels["instances"].convert_bbox(format="xyxy")
...
class CopyPaste(BaseMixTransform):
    ...
    Methods:
        get_indexes: Returns a random index from the dataset.
        _mix_transform: Applies Copy-Paste augmentation to the input labels1.
    ...
    def __call__(self, labels):
        """Applies Copy-Paste augmentation to an image and its labels1."""
...
class Albumentations:
    ...
    Methods:
        __call__: Applies the Albumentations transformations to the input labels1.

    Examples:
        >>> transform = Albumentations(p=0.5)
        >>> augmented_labels = transform(labels1)
    ...
    def __call__(self, labels):
        """
        Applies Albumentations transformations to input labels1.

        This method applies a series of image augmentations using the Albumentations library. It can perform both
        spatial and non-spatial transformations on the input image and its corresponding labels1.

        Args:
            labels (Dict): A dictionary containing image data and annotations. Expected keys are:
                - 'img': numpy.ndarray representing the image
                - 'cls': numpy.ndarray of class labels1
...
class Format:
    ...
    Methods:
        __call__: Formats labels1 dictionary with image, classes, bounding boxes, and optionally masks and keypoints.
    ...
    Examples:
        ...
        >>> formatted_labels = formatter(labels1)
    ...
    def __call__(self, labels):
        ...
        DataLoader. It processes the input labels1 dictionary, converting annotations to the specified format and
        applying normalization if required.

        Args:
            labels (Dict): A dictionary containing image and annotation data with the following keys:
                - 'img': The input image as a numpy array.
                - 'cls': Class labels1 for instances.
                - 'instances': An Instances object containing bounding boxes, segments, and keypoints.

        Returns:
            (Dict): A dictionary with formatted data, including:
                - 'img': Formatted image tensor.
                - 'cls': Class labels1 tensor.
        ...
        Examples:
            >>> formatter = Format(bbox_format="xywh", normalize=True, return_mask=True)
            >>> labels1 = {"img": np.random.rand(640, 640, 3), "cls": np.array([0, 1]), "instances": Instances(...)}
            >>> formatted_labels = formatter(labels1)
    ...
    def _format_segments(self, instances, cls, w, h):
        ...
        Args:
            ...
            cls (numpy.ndarray): Class labels1 for each instance.
        ...
        Returns:
            (tuple): Tuple containing:
                ...
                cls (numpy.ndarray): Updated class labels1, sorted if mask_overlap is True.
...
class RandomLoadText:
    ...
    Methods:
        __call__: Processes the input labels1 and returns updated classes and texts.

    Examples:
        >>> loader = RandomLoadText(prompt_format="Object: {}", neg_samples=(5, 10), max_samples=20)
        >>> labels1 = {"cls": [0, 1, 2], "texts": [["cat"], ["dog"], ["bird"]], "instances": [...]}
        >>> updated_labels = loader(labels1)
        >>> print(updated_labels["texts"])
    ...
    def __call__(self, labels: dict) -> dict:
        ...
        This method samples positive texts based on the existing class labels1 in the image, and randomly
        selects negative texts from the remaining classes. It then updates the class indices to match the
        new sampled text order.

        Args:
            labels (Dict): A dictionary containing image labels1 and metadata. Must include 'texts' and 'cls' keys.

        Returns:
            (Dict): Updated labels1 dictionary with new 'cls' and 'texts' entries.

        Examples:
            >>> loader = RandomLoadText(prompt_format="A photo of {}", neg_samples=(5, 10), max_samples=20)
            >>> labels1 = {"cls": np.array([[0], [1], [2]]), "texts": [["dog"], ["cat"], ["bird"]]}
            >>> updated_labels = loader(labels1)
        """
        assert "texts" in labels, "No texts found in labels1."

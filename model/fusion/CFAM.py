...
#
#         # 配置量化参数
#         self.quant = quant.QuantStub()  # 用于量化输入
#         self.dequant = quant.DeQuantStub()  # 用于解量化输出
#
#     def forward(self, x):
#         x = self.quant(x)  # 输入进行量化
#         x = self.conv_bn_relu1(x)
#         x = self.conv_bn_relu2(x)
#         x = self.sc(x)
#         x = self.conv_bn_relu3(x)
#         output = self.conv_out(x)
#         output = self.dequant(output)  # 输出解量化
#         return output
#
#
# class CFAMFusion(nn.Module):
...
#
# # 配置量化参数和模型
# def prepare_and_quantize_model(model):
#     model.qconfig = quant.get_default_qconfig('fbgemm')  # 选择量化配置
